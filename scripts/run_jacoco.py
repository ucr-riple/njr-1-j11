...
JACOCO_JAR = "tools/jacoco/jacocoagent.jar"
BENCHMARKS_FOLDER = "../../datasets/june2020_dataset/"
RESULTS_FOLDER = "results/jacoco_results"
JAVA_COMMAND = "java"
TIMEOUT = 1800
TIMEOUT_CMD = "timeout"
...
for benchmark in os.listdir(BENCHMARKS_FOLDER):
  print(benchmark)
  if (SKIP_COMPLETED):
        if os.path.exists(os.path.join(results_folder_path,(benchmark+".exec"))):
  ...
  wiretap_command = (TIMEOUT_CMD 
    + " " + str(TIMEOUT)
    + " " + JAVA_COMMAND
    + " -javaagent:" + JACOCO_JAR
    + "=destfile=" + RESULTS_FOLDER + "/" + benchmark + ".exec"
    + " -cp " + jarfile_path
    + " " + mainclass_name
  )
