...
public class NakedPairRow implements SolveMethod{
	...
	public void apply(Sudoku s, SolveStep ss) {
		...
		for (int c=0; c<s.getSize();c++){
			if ((s.getCell(row, c).getCandidates().contains(couple.a)
					||
					s.getCell(row, c).getCandidates().contains(couple.a))
					&&
					!nakedPair.contains(s.getCell(row, c))
			){
				s.getCell(row, c).removeCandidate(couple.a);
				s.getCell(row, c).removeCandidate(couple.b);
			}
		}
	...
	public void printInfo(SolveStep ss) {
		...
		List<Cell> nakedPair = (List<Cell>) ss.getParams().get("nakedPair");
		int row = nakedPair.get(0).getRow();
		System.out.println(String.format(getName() + ")There is a Naked pair (%s) in row %d", couple, row));
		System.out.println(String.format("Candidates %s can be removed from all cells of row %d but cells [%d,%d] and [%d,%d]",
				couple,row,
	...
	public SolveStep getStep(Sudoku s) {
		...
		for(int row=0; row<s.getSize(); row++){
			/*If a couple it's in EXACTLY two cells of the row and both candidates are not in other cells, there is a naked pair */
			for (Couple couple : allCouple) {
				List<Cell> nakedCells = new ArrayList<Cell>();
				for (int c=0; c<s.getSize();c++){/*Cicle through the row*/
					Cell tmp = s.getCell(row,c);
