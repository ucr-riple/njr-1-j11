//https://github.com/mgaut72/Mazes/blob/master/Ellers.java with mods

import java.util.*;

class	Maze{

	 static final RoomType	MAZE_WALL =	RoomType.WALL;
	 static final RoomType	MAZE_PATH =	RoomType.PATH;
	 ...
	 RoomType[][]	 feild;			  //the feild where the	maze is being made
	 ...
	 // Randomly pick	vertical	paths	for each	set, making	sure there is at least 1	vertical	path per	set
	 private	void makeVerticalCuts()
	 {
		  ...
		  boolean madeVertical;	 // tracks if a vertical path	has been	made in the section
	 ...
	 public void setStart(){
	 	for(int x = 0; x < feild[0].length; x++){
			if(feild[x][0] == RoomType.PATH){
				feild[x][1] = RoomType.START;
				feild[x][0] = RoomType.WALL;
			}
		}
	}
	 ...
	 public void setExit(){
	 	for(int x = 0; x < feild[0].length; x++){
			if(feild[x][feild[0].length-1] == RoomType.PATH){
				feild[x][feild[0].length-2] = RoomType.EXIT;
				feild[x][feild[0].length-1] = RoomType.WALL;
			}
		}
	}
	...
	public void setDoors(){
		...
		for(int i=1; i<rows-1; i++){
			for(int j=1; j<cols-1;	j++){
				Random rand = new Random();
				if(rand.nextDouble() < .1 && feild[i][j] == RoomType.WALL){
					feild[i][j] = RoomType.DOOR;
				}
			}
			System.out.println();
		 }		
