...
public enum CoreDownSchematic implements Schematic {
	DEFAULT(new String[] { "DVD", "#D#" });
	private final String[] map;
	private Map<Direction, List<Integer>> accessPoints = new EnumMap<Direction, List<Integer>>(
			Direction.class);
	private final Grid grid;

	CoreDownSchematic(String[] map) {
		this.map = map;
	...
	private void initialize_access(Direction dir) {
		...
		int x = (dir.isVertical()) ? 1 : ((dir == Direction.WEST) ? 0 : grid
				.getSize().getX() - 1);
		int y = (dir.isVertical()) ? ((dir == Direction.SOUTH) ? 0 : grid
				.getSize().getY() - 1) : 1;
		int sz = (dir.isVertical()) ? grid.getSize().getX() : grid.getSize()
				.getY();
		for (int o = 1; o < sz - 1; o++) {
			if (grid.get(x, y) == CoreMaterial.DOWN) {
				tmp.add(o);
					...
					// It's only a valid access point if the first square in the
					// room is also floor
	...
	public int getAccess(Direction oside, Direction dir) {
		...
		if (!tmp.isEmpty()) {
			...
			int max = (side.isVertical()) ? grid.getSize().getX() - 1 : grid
					.getSize().getY() - 1;
