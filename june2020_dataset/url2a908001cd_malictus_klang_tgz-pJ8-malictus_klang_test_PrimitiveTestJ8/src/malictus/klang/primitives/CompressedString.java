...
/**
 * Primitive representing a plain, unformatted string compressed using the ZLIB library.
 * @author Jim Halliday
 */
public class CompressedString implements Primitive, StringPrim {
	 ...
	 * just beyond the compressed string.
	 * 
	 * @param raf a RandomAccessFile object that points to a valid file, queued for reading
	 * @param stoppoint the byte position to stop reading data, or -1 if entire file should be read
	 ...
	 */
	public CompressedString(RandomAccessFile raf, long stoppoint, String textEncoding, boolean canChangeEncoding) throws IOException, BadValueException, CharacterCodingException {
		...
		raf.read(byt);
		byte[] inflated = KlangUtil.inflate(byt);
		try {
			value = new String(inflated, 0, inflated.length, textEncoding);
		} catch (Exception err) {
			err.printStackTrace();
	 ...
	 */
	public CompressedString(String value, String textEncoding, boolean canChangeEncoding) throws BadValueException {
		this.textEncoding = textEncoding;
		this.canChangeEncoding = canChangeEncoding;
	...
	public void writeValueToFile(RandomAccessFile raf) throws IOException, BadValueException, CharacterCodingException {
		...
		byte[] deflated = KlangUtil.deflate(theBytes);
		raf.write(deflated);
	}
