...
public class FMTChunk extends Chunk {
	...
	public FMTChunk(KlangFile parentFile, long startpos, long endpos, long datastartpos, long dataendpos, int chunkType, String chunkName, ContainerChunk parentChunk, Boolean isBigEndian, RandomAccessFile raf) throws IOException {
		...
		//have to skip the next two bytes (which merely indicate how many bytes are left)
		raf.skipBytes(2);
		if (keepgoing && (endpos >= (raf.getFilePointer() + 2))) {
			try {
				long bytepos = raf.getFilePointer();
		...
		//for now reading first two bytes of GUID only (which should indicate format)
		if (keepgoing && (endpos >= (raf.getFilePointer() + 2))) {
			try {
				long bytepos = raf.getFilePointer();
	...
	public void reparseChunkPrimitives(RandomAccessFile raff) throws IOException {
		...
		//check to see if the optional primitives are all null
		...
		//calculate needed byte size, varying depending on optional bytes
		int total = 0;
		counter = 0;
		...
		if (!isBasic) {
			//2 size bytes, plus additional 14 bytes at the end
			total = total + 16;
		}
		...
		if (!isBasic) {
			//write the end bytes
			//NOTE: not currently doing anything with these bytes!
