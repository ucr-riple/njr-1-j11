...
public class Leaf extends Node {
	...
	protected Node[] split(byte[] pData, Region mbr, int id) {
		...
		ArrayList<Integer> g1 = new ArrayList<Integer>(), g2 = new ArrayList<Integer>();
	...
	protected void deleteData(int id, Stack<Integer> pathBuffer, ArrayList<Integer> changedNodeID) {
		int child;
		//find the exact child
		...
		//maybe need reinsert some nodes
		...
		while (!toReinsert.empty()) {
			...
			for (int cChild = 0; cChild < n.m_children; cChild++) {
				...
				m_pTree.insertData_impl(n.m_pData[cChild], n.m_pMBR[cChild], n.m_pIdentifier[cChild], n.m_level, overflowTable, changedNodeID);
	...
	@Override
	public void drawSelf(Graphics g, double ratio) {
		drawRect(g, m_nodeMBR, COLOR_NODE, ratio, "" + m_identifier, COLOR_TEXT);
		for (int i = 0; i < m_children; i++)
			fillRect(g, m_pMBR[i], COLOR_DATA, ratio);
	}
}
