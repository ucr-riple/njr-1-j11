...
public abstract class Node implements INode {
	...
	static Color COLOR_NODE = Color.BLUE;
	static Color COLOR_DATA = Color.BLACK;
	static Color COLOR_QUERY = Color.ORANGE;
	static Color COLOR_TEXT = Color.RED;
	...
	/**
	 * The level of the node in the tree. Leaves are always at level 0.
	 */
	...
	/**
	 * The length of data stored in the node.
	 */
	...
	/**
	 * insert a entry into this node, child_num++
	 * 
	 * @param pData
	 * @param mbr
	 * @param id
	...
	protected void insertEntry(byte[] pData, Region mbr, int id) throws IllegalStateException {
		...
		// TODO Maybe this step should change to copy instead of point to.
	...
	protected void deleteEntry(int index) throws IndexOutOfBoundsException {
		...
		if (m_children > 1 && index != m_children - 1) {
			//replace the target entry with last enrty, then m_children--
	 ...
	 * @return
	 */
	protected boolean insertData(byte[] pData, Region mbr, int id, Stack<Integer> pathBuffer, boolean[] overflowTable, ArrayList<Integer> changedNodeID) {
		if (m_children < m_capacity) {
			boolean adjusted = false;
			boolean b = m_nodeMBR.contains(mbr);

			insertEntry(pData, mbr, id);
			m_pTree.writeNode(this);//save this change

			if (!b && !pathBuffer.empty()) {
				int cParent = (pathBuffer.pop()).intValue();
				Index p = (Index) m_pTree.readNode(cParent);
				p.adjustTree(this, pathBuffer);
				adjusted = true;
			}
			if (changedNodeID != null)
				changedNodeID.add(m_identifier);
			return adjusted;
			...
			ArrayList<Integer> vReinsert = new ArrayList<Integer>(), vKeep = new ArrayList<Integer>();
			...
			for (cIndex = 0; cIndex < lReinsert; cIndex++) {
				int i = (vReinsert.get(cIndex)).intValue();
			...
			for (cIndex = 0; cIndex < lKeep; cIndex++) {
				int i = (vKeep.get(cIndex)).intValue();
			...
			for (cIndex = 0; cIndex < lReinsert; cIndex++) {
				m_pTree.insertData_impl(reinsertdata[cIndex], reinsertmbr[cIndex], reinsertid[cIndex], m_level, overflowTable, changedNodeID);
			}
			...
			if (pathBuffer.empty()) {
				...
				if (changedNodeID != null) {
					changedNodeID.add(n.m_identifier);
					changedNodeID.add(nn.m_identifier);
					changedNodeID.add(r.m_identifier);
				}
			} else {
				...
				if (changedNodeID != null) {
					changedNodeID.add(n.m_identifier);
					changedNodeID.add(nn.m_identifier);
					changedNodeID.add(p.m_identifier);
				}
			}
	...
	protected void reinsertData(byte[] pData, Region mbr, int id, ArrayList<Integer> reinsert, ArrayList<Integer> keep) {
	...
	/**
	 * after inserted the new data, node should be split
	 * 
	 * @param pData
	 * @param mbr
	 * @param id
	 ...
	 * @param group2
	 */
	protected void rtreeSplit(byte[] pData, Region mbr, int id, ArrayList<Integer> group1, ArrayList<Integer> group2) {
		int cChild;
				...
				for (cChild = 0; cChild < m_capacity + 1; cChild++) {
					if (mask[cChild] == false) {
						...
						if (d > m) {
							...
							md2 = d2;
							sel = cChild;
							if (m_pTree.m_treeVariant == SpatialIndex.RtreeVariantLinear || m_pTree.m_treeVariant == SpatialIndex.RtreeVariantRstar)
								break;
						}
					}
				}
	...
	protected void rstarSplit(byte[] pData, Region mbr, int id, ArrayList<Integer> group1, ArrayList<Integer> group2) {
	...
	/**
	 * After delete node, the RTree maybe need reinsert some nodes. This method
	 * should be invoked to modify MBR or combine nodes after delete data at
	 * leaf node. This is a important step.
	 * 
	 * @param toReinsert
	 * @param pathBuffer
	 */
	protected void condenseTree(Stack<Node> toReinsert, Stack<Integer> pathBuffer) {
		...
		if (pathBuffer.empty()) {
			...
			if (m_level != 0 && m_children == 1) {
				...
				m_pTree.deleteNode(n);//delete the only child node
				n.m_identifier = m_pTree.m_rootID;
				m_pTree.writeNode(n);//rewrite root node
			...
			// the child-th entry of parent node is pointing to this node.
			int child;
	...
	/**
	 * write the data into byte[]
	 * 
	 * @return the byte that contains data
	 * @throws IOException
	 */
	...
	/**
	 * draw itself in a graphics
	 * 
	 * @param g
	 * @param ratio
	 *            the coordinate ratio
	 */
	protected abstract void drawSelf(Graphics g, double ratio);
	 ...
	 * @param ratio
	 */
	protected void drawRect(Graphics g, Region mbr, Color c, double ratio) {
		g.setColor(c);
		g.drawRect((int) (ratio * mbr.m_pLow[0]), (int) (ratio * mbr.m_pLow[1]), (int) (ratio * (mbr.m_pHigh[0] - mbr.m_pLow[0])),
				(int) (ratio * (mbr.m_pHigh[1] - mbr.m_pLow[1])));
	}

	protected void fillRect(Graphics g, Region mbr, Color c, double ratio) {
		if (g == null)
			return;
		g.setColor(c);
		g.fillRect((int) (ratio * mbr.m_pLow[0]), (int) (ratio * mbr.m_pLow[1]), (int) (ratio * (mbr.m_pHigh[0] - mbr.m_pLow[0])),
				(int) (ratio * (mbr.m_pHigh[1] - mbr.m_pLow[1])));
	}
	 ...
	 * @param ratio
	 */
	protected void drawRect(Graphics g, Region mbr, Color rectColor, double ratio, String str, Color strColor) {
		if (g == null)
			return;
		...
		g.drawRect((int) (ratio * mbr.m_pLow[0]), (int) (ratio * mbr.m_pLow[1]), (int) (ratio * (mbr.m_pHigh[0] - mbr.m_pLow[0])),
				(int) (ratio * (mbr.m_pHigh[1] - mbr.m_pLow[1])));
		g.setColor(strColor);
		g.drawString(str, (int) (ratio * (mbr.m_pLow[0] + (mbr.m_pHigh[0] - mbr.m_pLow[0]) / 2)),
				(int) (ratio * (mbr.m_pLow[1] + (mbr.m_pHigh[1] - mbr.m_pLow[1]) / 2)));
	}
	...
	class ReinsertEntryComparator implements Comparator<Object> {
		@Override
	...
	class RstarSplitEntryComparatorLow implements Comparator<Object> {
		@Override
		public int compare(Object o1, Object o2) {
	...
	class RstarSplitEntryComparatorHigh implements Comparator<Object> {
		@Override
		public int compare(Object o1, Object o2) {
			RstarSplitEntry e1 = (RstarSplitEntry) o1;
			RstarSplitEntry e2 = (RstarSplitEntry) o2;
