...
public class Region implements IShape, Serializable {
	...
	public boolean equals(Object o) {
		if (o instanceof Region) {
			...
			for (int cIndex = 0; cIndex < m_pLow.length; cIndex++) {
				if (m_pLow[cIndex] < r.m_pLow[cIndex] - EPSILON || m_pLow[cIndex] > r.m_pLow[cIndex] + EPSILON || m_pHigh[cIndex] < r.m_pHigh[cIndex] - EPSILON
						|| m_pHigh[cIndex] > r.m_pHigh[cIndex] + EPSILON) {
					return false;
				}
			}
			return true;
	...
	/**
	 * return relation code with another region, which code is described in
	 * IVisitor
	 * 
	 * @param r
	 * @return
	 */
	public int getRelationCode(IShape r) {
		if (this.intersects(r))
			return IVisitor.TYPE_INTERSECT;
		else if (this.contains(r))
			return IVisitor.TYPE_INSIDE;
		else
			return IVisitor.TYPE_OUTSIDE;
	}
	...
	public boolean touches(final Region r) {
		...
		for (int cIndex = 0; cIndex < m_pLow.length; cIndex++) {
			if ((m_pLow[cIndex] > r.m_pLow[cIndex] - EPSILON && m_pLow[cIndex] < r.m_pLow[cIndex] + EPSILON)
					|| (m_pHigh[cIndex] > r.m_pHigh[cIndex] - EPSILON && m_pHigh[cIndex] < r.m_pHigh[cIndex] + EPSILON)) {
				return true;
			}
		}
		return false;
	...
	public boolean touches(final Point p) {
		...
		for (int cIndex = 0; cIndex < m_pLow.length; cIndex++) {
			if ((m_pLow[cIndex] > p.m_pCoords[cIndex] - EPSILON && m_pLow[cIndex] < p.m_pCoords[cIndex] + EPSILON)
					|| (m_pHigh[cIndex] > p.m_pCoords[cIndex] - EPSILON && m_pHigh[cIndex] < p.m_pCoords[cIndex] + EPSILON)) {
				return true;
			}
		}
		return false;
	...
	/**
	 * 
	 * @return the margin of a region. It is calcuated as the sum of 2^(d-1) *
