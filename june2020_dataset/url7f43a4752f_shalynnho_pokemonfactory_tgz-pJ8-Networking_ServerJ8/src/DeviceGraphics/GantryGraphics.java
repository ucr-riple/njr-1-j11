...
public class GantryGraphics implements DeviceGraphics, GraphicsInterfaces.GantryGraphics {
	...
	public void receiveBin(Bin newBin, FeederAgent feeder) {
		...
		newLocation.incrementX(65);
		newLocation.incrementY(40);
	...
	private void moveTo(Location newLocation) {
		server.sendData(new Request(Constants.GANTRY_ROBOT_MOVE_TO_LOC_COMMAND, Constants.GANTRY_ROBOT_TARGET,
				newLocation));
	}
	...
	public void receiveData(Request req) {
		if (req.getCommand().equals(Constants.GANTRY_ROBOT_DONE_MOVE)) {
			...
			if (receiveState) {
				server.sendData(new Request(Constants.GANTRY_ROBOT_GET_BIN_COMMAND, Constants.GANTRY_ROBOT_TARGET,
						new BinData(heldBin.binGraphics.getLocation(), heldBin.part.type)));
				moveTo(newLocation);
			...
			// Robot is above initial bin location, removes bin
				...
				server.sendData(new Request(Constants.GANTRY_ROBOT_DROP_BIN_COMMAND, Constants.GANTRY_ROBOT_TARGET,
						null));
	...
	public void hereIsNewBin(Bin bin) {
		...
		server.sendData(new Request(Constants.GANTRY_ROBOT_ADD_NEW_BIN, Constants.GANTRY_ROBOT_TARGET, new BinData(
				bin.binGraphics.getInitialLocation(), bin.part.type)));
	}
	...
	public void editBin(PartType pt) {
		for (BinGraphics bin : binList) {
			if (bin.getPart().getPartType().equals(pt)) {
				...
				server.sendData(new Request(Constants.GANTRY_ROBOT_EDIT_BIN, Constants.GANTRY_ROBOT_TARGET,
						new BinData(null, pt)));
			}
		}
