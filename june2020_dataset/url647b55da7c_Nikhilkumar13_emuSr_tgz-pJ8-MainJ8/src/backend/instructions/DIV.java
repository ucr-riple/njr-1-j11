 ...
 * @author ROCK
 */
public class DIV implements InsInterface4
{
   ...
   public void execute(String ins, String token2, String token3, String token4)
      {
        ...
        if (no > -1)
        {
           ...
           if(no2>-1)
           {
                ...
                {
                    FrontEnd.appendToPane(FrontEnd.statuswindow,"ERROR(" +ScanFile.current_line+ "):" + " Failed To Execute DIV Instruction" + "\n",Color.RED);
...
//                frontend.FrontEnd.statuswindow.append("ERROR in line " + backend.ScanFile.current_line + ":" + "The destination operand of AND should be a register and " + token2 + " isnot a valid register.");
                //frontend.FrontEnd.statuswindow.setCaretPosition(frontend.FrontEnd.statuswindow.getText().length());
               FrontEnd.appendToPane(FrontEnd.statuswindow,"ERROR(" +ScanFile.current_line+ "):" + " Invalid Ist Source Register For DIV Instruction\n",Color.RED);
            }
          ...
          {
            FrontEnd.appendToPane(FrontEnd.statuswindow,"ERROR(" + backend.ScanFile.current_line + "): Invalid Destination Register For DIV Instruction  \n",Color.RED);
